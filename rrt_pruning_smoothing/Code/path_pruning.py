...
def prunedPath(path, radius, clearance):
	...
	while (j != 0):
		...
		for i in range(0, j):
			...
			# print("x1, y1: ", x1, y1)
			# print("x2, y2: ", x2, y2)
			status = isObstacleLine(np.array([x2, y2]), np.array([x1, y1]), radius, clearance)
			# print("status: ", status)
			if (status is False):
				# print("i: ", i)
...
def isObstacleLine(start, end, radius, clearance):
	...
	for t in pts:
		...
		eqn = t * start + (1 - t) * end
		# print(eqn)
		obs.append(obstacles.withinObstacleSpace(point=eqn, radius=radius, clearance=clearance))
