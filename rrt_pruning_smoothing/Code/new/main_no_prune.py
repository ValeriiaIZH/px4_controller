...
def rrtPlannedPath(start_node, goal_node, robot_radius, plotter, write=False):
	...
	while (not utils.sameRegion(step_node, goal_node, GOAL_REACH_THRESH)) and (itr < MAX_ITER):
		...
		if plotter is not None:
			utils.plotPoint(rand_node.getXYCoords(), plotter, radius=0.1, color='red')
		...
		if rrt.hitsObstacle(start_node=closest_node, goal_node=step_node, step_size=(robot_radius/2)):
			if plotter is not None:
				utils.plotPoint(step_node.getXYCoords(), plotter, radius=0.2, color='red')
				cn_x, cn_y = closest_node.getXYCoords()
				sn_x, sn_y = step_node.getXYCoords()
...
def main():
	start_node = node_rrt.Node_rrt(current_coords=(-7, -7), parent_coords=None, distance=0)
	goal_node = node_rrt.Node_rrt(current_coords=(7, 107), parent_coords=None, distance=0)

	fig, ax = plt.subplots()
	ax.set_xlim(-10, 10)
	ax.set_ylim(-10, 110)
	fig.gca().set_aspect('equal', adjustable='box')
	utils.plotPoint(start_node.getXYCoords(), ax, radius=0.5, color='cyan') 	# start
	utils.plotPoint(goal_node.getXYCoords(), ax, radius=0.5, color='magenta') 	# end
